...
int set_wifi_bt_fast_clk(int on)
{
	...
	if (1 == run_mode) {
		if (on) {
			...
			err = vreg_enable(vreg_osc);
			printk(KERN_DEBUG "EN FAST CLK: %d\n", err);
		} else {
			printk(KERN_DEBUG "DIS FAST CLK\n");
			err = vreg_disable(vreg_osc);
			printk(KERN_DEBUG "DIS FAST CLK: %d\n", err);
		}
	} else if (2 == run_mode) {
		if (on) {
			gpio_set_value(htc_fast_clk_pin, 1);
			printk(KERN_DEBUG "EN FAST CLK\n");
		} else {
			gpio_set_value(htc_fast_clk_pin, 0);
			printk(KERN_DEBUG "DIS FAST CLK\n");
		}
	} else {
...
int htc_wifi_bt_fast_clk_ctl(int on, int id)
{
	...
	if ((htc_fast_clk_pin < 0) && (vreg_osc == NULL)) {
		printk(KERN_DEBUG "== ERR FAST CLK PIN 7K vreg_osc ==\n");
		return -EINVAL;
	}
...
static int htc_fast_clk_probe(struct platform_device *pdev)
{
	...
	if (pdata == NULL) {
		...
		if (1 == pdata->mode) {
			...
			vreg_osc = vreg_get(NULL, pdata->id);
			if (IS_ERR(vreg_osc)) {
