...
#include "../inc/si_api_configure.h"
#include "si_regio_cbus.h"
#include "si_cbus_defs.h"
...
enum
{
	MHD_MSC_MSG_STATE_CHANGE             	= 0x81,
	MHD_MSC_MSG_DEVCAP0_CHANGE            	= 0x82,
	MHD_MSC_MSG_DEVCAP1_CHANGE            	= 0x83,
	MHD_MSC_MSG_DEVCAP2_CHANGE            	= 0x84,
	MHD_MSC_MSG_DEVCAP3_CHANGE            	= 0x85,
	MHD_MSC_MSG_DEVCAP4_CHANGE            	= 0x86,
	MHD_MSC_MSG_DEVCAP5_CHANGE            	= 0x87,
	MHD_MSC_MSG_DEVCAP6_CHANGE            	= 0x88,
	MHD_MSC_MSG_DEVCAP7_CHANGE            	= 0x89,
	MHD_MSC_MSG_DEVCAP8_CHANGE            	= 0x8A,
	MHD_MSC_MSG_DEVCAP9_CHANGE            	= 0x8B,
	MHD_MSC_MSG_DEVCAP10_CHANGE            	= 0x8C,
	MHD_MSC_MSG_DEVCAP11_CHANGE            	= 0x8D,
	MHD_MSC_MSG_DEVCAP12_CHANGE            	= 0x8E,
	MHD_MSC_MSG_DEVCAP13_CHANGE            	= 0x8F,
	MHD_MSC_MSG_DEVCAP14_CHANGE            	= 0x90,
	MHD_MSC_MSG_DEVCAP15_CHANGE            	= 0x9A,
	MHD_MSC_MSG_SET_INT0_CHECK		= 0x9B,
	MHD_MSC_MSG_SET_INT1_CHECK		= 0x9C,
	MHD_MSC_MSG_SET_INT2_CHECK		= 0x9D,
	MHD_MSC_MSG_SET_INT3_CHECK		= 0x9E,
	MHD_MSC_MSG_WRITE_STAT0_CHECK		= 0x9F,
	MHD_MSC_MSG_WRITE_STAT1_CHECK		= 0xA0,
	MHD_MSC_MSG_WRITE_STAT2_CHECK		= 0xA1,
	MHD_MSC_MSG_WRITE_STAT3_CHECK		= 0xA2,
	MHD_MSC_MSG_SET_INT_REPLY		= 0xA3,
	MHD_MSC_MSG_WRITE_STAT_REPLY		= 0xA4
...
enum
{
	...
	GET_SC1_ERRORCODE			= 0x69,	// Get channel 1 command error code
	GET_DDC_ERRORCODE			= 0x6A,	// Get DDC channel command error code.
	GET_MSC_ERRORCODE			= 0x6B,	// Get MSC command error code.
	MHD_WRITE_BURST				= 0x6C,	// Write 1-16 bytes to responder’s scratchpad.
	GET_SC3_ERRORCODE			= 0x6D,	// Get channel 3 command error code.
};
...
typedef struct
{
    ...
    uint8_t msgData[MHD_MAX_BUFFER_SIZE];   // Pointer to message data area.
...
typedef struct
{
    ...
    uint8_t state;          // State of command execution for this channel
                            // IDLE, WAIT_RESPONSE
    uint8_t timeout;        // Timer for message response (must be able to indicate at least 3000ms).
...
void        SI_CbusHeartBeat( uint8_t chn, uint8_t enable);
...
uint8_t		SI_CbusMscReturnCmd(void);
uint8_t		SI_CbusMscReturnValue(void);
void 		SI_CbusMscResetReturnCmd(void);
void 		SI_CbusMscResetReturnValue(void);
