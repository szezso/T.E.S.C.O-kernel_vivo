...
static DEVICE_ATTR(video_mode, S_IRUGO | S_IWUSR, hdmi_common_rda_video_mode,
...
{
	int rc;
	struct fb_info *fbi = registered_fb[1];
	...
	hdmi_common_state->uevent_kobj = &fbi->dev->kobj;
...
static uint32 hdmi_edid_hdmi_sink(const uint8 *in_buf)
{
        uint8 len;
	bool ret = false;
        const uint8 *sink = hdmi_edid_find_block(in_buf, 3, &len);

        while (len >= 3) {
		if (sink[0] == 0x03 && sink[1] == 0x0c &&
			sink[2] == 0x00) {
			ret = true;
			break;
		}
...
static void hdmi_edid_get_display_mode(const uint8 *data_buf,
	struct hdmi_disp_mode_list_type *disp_mode_list,
	uint32 num_og_cea_blocks, bool short_desc)
{
...
static bool check_edid_header(const uint8 *edid_buf)
{
...
int hdmi_common_read_edid(void)
{
	...
	case 0: /* No CEA extension */
		hdmi_common_state->hdmi_sink = false;
		DEV_DBG("HDMI DVI mode: %s\n", hdmi_sink?"no":"yes");
		break;
	case 1: /* Read block 1 */
		...
		hdmi_common_state->hdmi_sink =  hdmi_edid_hdmi_sink(edid_buf+0x80);
		break;
	default:
...
#if 0 
void hdmi_common_get_video_format_from_drv_data(struct mdp_info *mdp)
{
	uint32 format;
	struct fb_var_screeninfo *var = &mdp->mdp_dev.fbi->var;

	if (var->reserved[3]) {
		format = var->reserved[3]-1;
	} else {
		DEV_DBG("detecting resolution from %dx%d use var->reserved[3]"
			" to specify mode", var->xres, var->yres);
		switch (var->xres) {
		default:
		case  640:
